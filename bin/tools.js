...
const COMPOUND_TOOLS = [
  ...
  { type: 'function', function: { name: 'tdd_loop', description: 'Start a TDD loop for a list of requirements. The agent will work through each requirement using Red→Green→(Refactor), automatically advancing to the next after each one goes green. The loop only completes when every requirement has a passing test AND a clean full-suite run passes. Use this as the entry point for feature work driven by acceptance criteria.', parameters: { type: 'object', properties: { requirements: { type: 'array', items: { type: 'string' }, description: 'List of requirements to implement, each as a plain-English behaviour description (e.g. "add() returns the sum of two numbers", "subtract() raises ValueError on non-numeric input").' } }, required: ['requirements'] } } },
  { type: 'function', function: { name: 'tdd_begin_cycle', description: 'Start a TDD cycle for a named test (enters RED phase). In loop mode, call this after writing the failing test for the current requirement. In single-cycle mode, call this for any test.', parameters: { type: 'object', properties: { test_name: { type: 'string', description: 'The test name or identifier to track (e.g. "test_add" or "adds two numbers correctly").' } }, required: ['test_name'] } } },
  { type: 'function', function: { name: 'tdd_status', description: 'Show the current TDD phase, active requirement, requirements checklist, and what action is required next.', parameters: { type: 'object', properties: {}, required: [] } } },
  { type: 'function', function: { name: 'tdd_advance', description: 'Advance the TDD cycle to the next phase. RED (confirmed) → runs tests to check green. GREEN → enter REFACTOR (or skip_refactor=true to complete directly). REFACTOR → runs tests, completes cycle, advances loop to next requirement. Always call run_tests before tdd_advance to have current results.', parameters: { type: 'object', properties: { skip_refactor: { type: 'boolean', description: 'Skip REFACTOR and complete the cycle directly from GREEN (loop advances immediately).' } }, required: [] } } },
  { type: 'function', function: { name: 'tdd_reset', description: 'Reset the TDD state machine to idle, clearing all requirements and the active cycle. Use if the loop or cycle needs to be restarted from scratch.', parameters: { type: 'object', properties: {}, required: [] } } },
];
